...
connectToContract(config).then(function(connectionPool){
  ...
  log.info('- connection to fabric network ready for:')
  connectionPool.forEach(connection => {
    log.info('-- '+connection.gateway.currentIdentity._name)  
  })

  //---------------------------------
  // baue die DB Verbindung auf 
  //---------------------------------
  DB.init(config, log).then(function(result){
    if(!result){
      throw new Error("DB startimg error")
    }

    let database = DB.getDatabase('app3');
    log.info('Database app3 ready')
      ...
      app.post('/getCrop', async function (req, res) {
        let result = await getCrop(req, connectionPool, helper, log, database)
        res.json(result);
      })
      ...
      app.post('/createCrop', async function (req, res) {
        let result = await createCrop(req, connectionPool, helper, log, database)
        res.json(result);
      })
